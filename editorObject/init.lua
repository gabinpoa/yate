...
function game:getSomeLineLen(line)
    return #self.txt[line]
end

function game:unsafeSetCursorPos(pos)
    self.cursor.pos = pos
    self:updateLimitLines()
end
...
function game:unsafeSetCursorLine(line)
...
function game:displayTextLineByLine()
    for i = self.window.startLine, self.window.endLine do
        local line = self.txt[i]
